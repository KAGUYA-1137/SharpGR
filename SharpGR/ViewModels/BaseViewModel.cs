...
namespace SharpGR.ViewModels
{
    ...
    /// </summary>
    /// <remarks>
    /// <see cref="DispatcherObject"/> を継承し、<see cref="INotifyPropertyChanged"/> を実装することで
    /// プロパティ変更通知の共通処理を提供します。
    /// ビューにバインドするプロパティの setter では本クラスの <see cref="SetProperty{T}"/> を利用して
    /// 値の変更と通知を行ってください。
    /// </remarks>
    public class BaseViewModel : DispatcherObject, INotifyPropertyChanged
    {
        ...
        /// </summary>
        /// <remarks>
        /// プロパティが変更されたときに発生します。通常は <see cref="SetProperty{T}"/> メソッドを通じて発行されます。
        ...
        /// 変更されたプロパティの名前。デフォルトでは呼び出し元のメンバー名が自動的に設定されます（<see cref="CallerMemberName"/>）。
        /// </param>
        /// <returns>
